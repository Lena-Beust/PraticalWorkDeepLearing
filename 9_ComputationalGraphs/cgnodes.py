...
class MSELossNode(MetaNode):
    """A MSE loss node: (1/(2*N)) * np.sum((y - y_hat)**2)
    """

    def __init__(self, x1: ValueNode, x2: ValueNode, N: int, out: ValueNode):
        """Create a mse loss operator node.
    ...
    def forward(self):
        ...
        if self.input_ready:
            x1_val, x2_val = self.get_parent_values()
            z = (1/(2*self.N)) * np.sum((x1_val - x2_val)**2)   # MSE
            for node in self.children:
                node.receive_parent_value(z)
                node.forward()
